...
def get_best_segments(
    all_relevance_values: list[list],
    document_splits: list[int],
    max_length: int,
    overall_max_length: int,
    minimum_value: float,
):
    """This function takes the chunk relevance values and then runs an optimization
    algorithm to find the best segments.
    ...
    while total_length < overall_max_length:
        ...
        relevance_values = all_relevance_values[
            rv_index
        ]  # get the relevance values for this query
        best_segment = None
        best_value = -1000
        for start in range(len(relevance_values)):
            ...
            if relevance_values[start] < 0:
                continue
            for end in range(
                start + 1, min(start + max_length + 1, len(relevance_values) + 1)
            ):
...
def get_meta_document(
    all_ranked_results: list[list], top_k_for_document_selection: int
):
    ...
    for ranked_results in all_ranked_results:
        top_document_ids.extend(
            [
                result["metadata"]["doc_id"]
                for result in ranked_results[:top_k_for_document_selection]
            ]
        )  # get document IDs for top results for each query
    unique_document_ids = list(
        set(top_document_ids)
    )  # get the unique document IDs for the top results across all queries
        ...
        for ranked_results in all_ranked_results:
            for result in ranked_results:
                if result["metadata"]["doc_id"] == document_id:
                    max_chunk_index = max(
                        max_chunk_index, result["metadata"]["chunk_index"]
                    )
        document_start_points[document_id] = (
            document_splits[-1] if document_splits else 0
        )
        document_splits.append(
            int(
                max_chunk_index + document_splits[-1] + 1
                if document_splits
                else max_chunk_index + 1
            )
        )  # basically the start point of the next document
...
def get_chunk_value(chunk_info: dict, irrelevant_chunk_penalty: float, decay_rate: int):
    ...
    absolute_relevance_value = chunk_info.get(
        "absolute_relevance_value", 0.0
    )  # if absolute_relevance_value is not provided, default to 0.0
...
def get_relevance_values(
    ...
    for ranked_results in all_ranked_results:
        ...
        for rank, result in enumerate(ranked_results):
            ...
            meta_document_index = int(
                document_start_points[document_id] + chunk_index
            )  # find the correct index for this chunk in the meta-document
        ...
        relevance_values = [
            get_chunk_value(chunk_info, irrelevant_chunk_penalty, decay_rate)
        ...
        if chunk_length_adjustment:
            ...
            chunk_lengths = [
                chunk_info.get("chunk_length", 0.0) for chunk_info in all_chunk_info
            ]
            relevance_values = adjust_relevance_values_for_chunk_length(
                relevance_values, chunk_lengths
            )
...
def adjust_relevance_values_for_chunk_length(
    relevance_values: list[float], chunk_lengths: list[int], reference_length: int = 700
):
    """
    ...
    """
    assert len(relevance_values) == len(chunk_lengths), (
        "The length of relevance_values and chunk_lengths must be the same"
    )
    adjusted_relevance_values = []
    for relevance_value, chunk_length in zip(relevance_values, chunk_lengths):
        bounded_chunk_length = max(
            chunk_length, reference_length
        )  # only adjust relevance values for chunks that are longer than the reference length
        adjusted_relevance_values.append(
            relevance_value * (bounded_chunk_length / reference_length)
        )
