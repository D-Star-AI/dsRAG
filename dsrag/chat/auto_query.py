...
def validate_queries(queries: List[Query], kb_info: list[dict]) -> List[dict]:
    """Validates and potentially modifies search queries based on knowledge base
    availability.
    ...
    for query in queries:
        if query.knowledge_base_id in valid_kb_ids:
            validated_queries.append(
                {"query": query.query, "kb_id": query.knowledge_base_id}
            )
        else:
            ...
            if len(kb_info) == 1:
                ...
                validated_queries.append(
                    {"query": query.query, "kb_id": kb_info[0]["id"]}
                )
            else:
...
def get_search_queries(
    ...
    max_queries: int = 5,
    auto_query_model: str = "gpt-4o-mini",
    ...
    try:
        ...
        fallback_model = (
            "claude-3-5-sonnet-20241022" if "gpt" in auto_query_model else "gpt-4o"
        )
        queries = get_response(
            messages=messages,
    ...
    validated_queries = validate_queries(queries.queries[:max_queries], kb_info)[
        :max_queries
    ]
    return validated_queries
