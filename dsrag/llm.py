...
class LLM(ABC):
    ...
    def make_llm_call(self, chat_messages: list[dict]) -> str:
        """Takes in chat_messages (OpenAI format) and returns the response from the LLM
        as a string."""
        pass
    ...
    def make_llm_call(self, chat_messages: list[dict]) -> str:
        ...
        if base_url is not None:
            client = anthropic.Anthropic(
                api_key=os.environ["ANTHROPIC_API_KEY"], base_url=base_url
            )
        else:
...
class GeminiAPI(LLM):
    def __init__(
        ...
        self.model_name = (
            model  # Renamed to avoid conflict with genai.GenerativeModel instance
        )
        self.temperature = temperature
        self.max_tokens = max_tokens
        # Configure API key
        ...
        try:
            ...
            print(
                f"Warning: Could not configure genai, possibly already configured: {e}"
            )
            pass
        ...
        try:
            ...
            if hasattr(response, "text"):
                ...
                if response.candidates and response.candidates[0].content.parts:
                    ...
                    print(
                        f"Warning: Gemini API call returned no content. Finish Reason: {finish_reason}"
                    )
