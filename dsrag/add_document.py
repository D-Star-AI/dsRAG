...
def auto_context(
    ...
    ingestion_logger.debug(
        "Starting AutoContext step", extra={**base_extra, "step": "auto_context_start"}
    )
            ...
            for future in as_completed(future_to_section):
                ...
                try:
                    ...
                    ingestion_logger.error(
                        f"Error processing section {i}: {str(e)}",
    ...
    if auto_context_config.get("custom_term_mapping", None):
        ...
        ]  # need to convert chunks to list of strings
        annotated_chunks = annotate_chunks(
            raw_chunks, auto_context_config["custom_term_mapping"]
        )
    ...
    for i, chunk in enumerate(chunks):
        ...
        if auto_context_config.get("custom_term_mapping", None):
            chunk["content"] = annotated_chunks[
                i
            ]  # override the chunk content with the annotated content if custom term mapping is used
        chunk_to_embed = f"{chunk_header}\n\n{chunk['content']}"
        chunks_to_embed.append(chunk_to_embed)

    step_duration = time.perf_counter() - step_start_time
    ingestion_logger.debug(
        "AutoContext complete",
        extra={
            **base_extra,
            "step": "auto_context",
...
def get_embeddings(embedding_model: Embedding, chunks_to_embed):
    ...
    for i in range(0, len(chunks_to_embed), 50):
        chunk_embeddings += embedding_model.get_embeddings(
